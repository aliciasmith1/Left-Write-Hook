...
d2[, 15:114] <- lapply(d2[, 15:114], as.numeric)
...
## ----PTSD dx - descriptive statistics-----------------------------------------

# PTSD: started with PTSD, check how many lost it
df_ptsd_wide <- full_d %>%
  subset(select = c("id", "group", "time.point", "PTSD_dx")) %>%
  pivot_wider(names_from = time.point, values_from = PTSD_dx,
              names_sep = "_") %>%
  filter(Pre == 1)

n_ptsd_pre <- nrow(df_ptsd_wide)
n_ptsd_lost <- sum(df_ptsd_wide$Post == 0, na.rm = TRUE)
PTSD_prop <- (n_ptsd_lost / n_ptsd_pre) * 100

cat("PTSD: ", n_ptsd_lost, "out of", n_ptsd_pre,
    "no longer meet criteria at post (", round(PTSD_prop, 1), "%).\n", sep=" ")

# CPTSD: started with CPTSD, check how many lost it
df_cptsd_wide <- full_d %>%
  subset(select = c("id", "group", "time.point", "CPTSD_dx")) %>%
  pivot_wider(names_from = time.point, values_from = CPTSD_dx,
              names_sep = "_") %>%
  filter(Pre == 1)

n_cptsd_pre <- nrow(df_cptsd_wide)
n_cptsd_lost <- sum(df_cptsd_wide$Post == 0, na.rm = TRUE)
CPTSD_prop <- (n_cptsd_lost / n_cptsd_pre) * 100

cat("CPTSD: ", n_cptsd_lost, "out of", n_cptsd_pre,
    "no longer meet criteria at post (", round(CPTSD_prop, 1), "%).\n", sep=" ")

# Of those with CPTSD at pre, check post status
df_2ptsd_wide <- full_d %>%
  subset(select = c("id", "group", "time.point", "CPTSD_dx", "PTSD_dx")) %>%
  pivot_wider(names_from = time.point, values_from = c(PTSD_dx, CPTSD_dx),
              names_sep = "_") %>%
  filter(CPTSD_dx_Pre == 1)

n_cptsd_pre2 <- nrow(df_2ptsd_wide)
n_cptsd_to_ptsd <- sum(df_2ptsd_wide$PTSD_dx_Post == 1 & df_2ptsd_wide$CPTSD_dx_Post == 0, na.rm = TRUE)
n_cptsd_none <- sum(df_2ptsd_wide$PTSD_dx_Post == 0 & df_2ptsd_wide$CPTSD_dx_Post == 0, na.rm = TRUE)

CPTSD_PTSD_prop <- (n_cptsd_to_ptsd / n_cptsd_pre2) * 100
CPTSD_none_prop <- (n_cptsd_none / n_cptsd_pre2) * 100

cat("Of those with CPTSD at pre:\n",
    " - ", n_cptsd_to_ptsd, " transitioned to PTSD only at post (", round(CPTSD_PTSD_prop, 1), "%)\n",
    " - ", n_cptsd_none, " had no diagnosis at post (", round(CPTSD_none_prop, 1), "%)\n",
    sep="")



df_long <- full_d %>%
  dplyr::select(id, time.point, PTSD_dx, CPTSD_dx) %>%
  mutate(
    diagnosis = case_when(
      CPTSD_dx == 1 ~ "CPTSD",
      PTSD_dx == 1 ~ "PTSD",
  ...
  mutate(diagnosis = factor(diagnosis,
                            levels = c("No diagnosis", "PTSD", "CPTSD")))

ggplot(df_long, aes(x = time.point, fill = diagnosis)) +
  geom_bar(position = "fill") +
  scale_y_continuous(labels = scales::percent) +
  scale_fill_manual(values = c("orange", "gold", "khaki1")) + 
  labs(x = "Time", y = "Percentage of participants", fill = "Diagnosis") +
  theme_minimal()
...
PTSD_plot <- 
  ggarrange(ITQ_PTSD_plot, ITQ_DSO_plot,
         labels = c("A", "B"),
         ncol = 2, nrow = 1)
PTSD_plot
...
DASS_plot <- ggplot(data = full_d, aes(x= time.point, y= DASS_total, fill = time.point))+
  stat_summary(geom = "bar", fun = mean, position = "dodge", show.legend = F) + 
  stat_summary(geom = "errorbar", fun.data = mean_se, position = "dodge", width = 0.3)+
  scale_fill_manual(values = c("#4fa8dc", "#08306b")) + 
  theme_classic() +
  theme(plot.title = element_text(size = 12)) +
  expand_limits(y=0) +
  ggtitle("Depression, Anxiety, Stress") +
...
ggarrange(ITQ_PTSD_plot, ITQ_DSO_plot, ncol = 2, nrow = 1)

ggarrange(TMQQ_plot, PTCI_plot, ncol = 2, nrow = 1)

ggarrange(DASS_plot, sMHC_plot, ncol = 2, nrow = 1)

ggarrange(vig_PE_plot, mod_PE_plot, walk_plot, ncol = 3, nrow = 1)
